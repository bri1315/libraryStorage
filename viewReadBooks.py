...
#just added a sort by owned and not owned
import tkinter
from tkinter import *
...
def sortByTitle():
        try:
            cur.execute("SELECT title, author, haveIRead, owned, price FROM library WHERE haveIRead = 'yes' ORDER BY title")
            #result = cur.fetchall()
            i = 4
            for library in cur:
...
def sortByAuthor():
        try:
            cur.execute("SELECT title, author, haveIRead, owned, price FROM library WHERE haveIRead = 'yes' ORDER BY author")
            #result = cur.fetchall()
            i = 4
            for library in cur:
                for j in range(len(library)):
...
def sortByOwned():
    try:
        cur.execute("SELECT title, author, haveIRead,owned, price FROM library WHERE haveIRead = 'yes' AND owned = 'yes' ORDER BY title")
        #result = cur.fetchall()
        i = 4
        for library in cur:
            for j in range(len(library)):
...
def sortByNotOwned():
    try:
        cur.execute("SELECT title, author, haveIRead,owned, price FROM library WHERE haveIRead = 'yes' AND owned = 'no' ORDER BY title")
        #result = cur.fetchall()
        i = 4
        for library in cur:
            for j in range(len(library)):
...
def viewReadBooks():
    ...
    viewTitle = Label(root, text = 'View Books You Read',  fg = "white",
